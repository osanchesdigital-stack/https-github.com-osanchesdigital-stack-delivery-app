...
const PopularItems = React.lazy(() => import('@/components/PopularItems'));
...
const MeuDelivery: React.FC = () => {
    ...
    // Establishment state for header and admin panel
    const [establishment, setEstablishment] = useState<{ name: string; logoUrl: string }>({ name: 'Meu Delivery', logoUrl: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80"><rect width="100%25" height="100%25" fill="%23e2e8f0"/><text x="50%25" y="50%25" dominant-baseline="middle" text-anchor="middle" fill="%23333" font-family="Arial" font-size="20">MD</text></svg>' });
    useEffect(() => {
    ...
    useEffect(() => {
        const fetchFromSupabase = async () => {
            const { data: categoriesData } = await supabase.from('categories').select('id,name').limit(12);
            if (categoriesData && categoriesData.length > 0) {
                const iconMap: Record<string, string> = { Carnes: 'kebab_dining', Bebidas: 'local_bar', Saladas: 'grass', Promo: 'local_offer' };
                const mappedCategories: Category[] = categoriesData.map((c: any) => ({ id: String(c.id), name: c.name, icon: iconMap[c.name] || 'category' }));
                setCategories(mappedCategories);
            }

            const { data: productsData } = await supabase.from('products').select('id,name,description,price,image_url').limit(24);
            if (productsData && productsData.length > 0) {
                const mappedItems: PopularItem[] = productsData.map((p: any) => ({
                    id: String(p.id),
                    name: p.name,
                ...
                setFavoriteItems(mappedItems.slice(0, Math.min(4, mappedItems.length)));
            }

            const { data: estData } = await supabase.from('establishments').select('name,logo_url').limit(1).maybeSingle();
            if (estData) {
                setEstablishment({ name: estData.name || establishment.name, logoUrl: estData.logo_url || establishment.logoUrl });
            }
        };
        fetchFromSupabase();
    }, []);
    ...
    const handleNavigateToManageMenu = useCallback(() => {
        setCurrentView('manageMenu');
    }, []);
    
    const handleNavigateToSuperAdmin = useCallback(() => {
        setCurrentView('superAdmin');
    }, []);

    const handleBottomNavClick = useCallback((view: View) => {
        setCurrentView(view);
    }, []);
    
    const handleSideMenuNavigate = useCallback((view: 'home' | 'pedidos' | 'perfil' | 'enderecos' | 'paymentMethods') => {
        setCurrentView(view);
        setIsMenuOpen(false);
    }, []);
    ...
    const handleAddToCart = useCallback((item: PopularItem, quantity: number, customization: ProductCustomization) => {
    ...
    const handleUpdateQuantity = useCallback((itemId: string, amount: number) => {
    ...
    const SkeletonSection: React.FC<{ title?: string }> = ({ title }) => (
        <section className="px-4 pt-4 pb-4 animate-pulse">
            {title && <div className="h-6 w-40 bg-black/10 dark:bg-white/10 rounded mb-3" />}
            <div className="flex gap-4">
                <div className="h-24 w-24 bg-black/10 dark:bg-white/10 rounded" />
                <div className="flex-1 space-y-2">
                    <div className="h-4 w-3/5 bg-black/10 dark:bg-white/10 rounded" />
                    <div className="h-4 w-2/5 bg-black/10 dark:bg-white/10 rounded" />
    ...
    const homeContent = useMemo(() => (
        <>
            ...
            <Suspense fallback={<SkeletonSection title="Categorias" />}>
                <Categories categories={categories} />
            </Suspense>
            <Suspense fallback={<SkeletonSection title="Queridinhos" />}>
                <Favorites items={favoriteItems} onSelectItem={handleSelectItem} />
            </Suspense>
            <Suspense fallback={<SkeletonSection title="Mais Populares" />}>
                <PopularItems items={popularItems} onSelectItem={handleSelectItem} />
            </Suspense>
        </>
    ), [carouselImages, categories, favoriteItems, popularItems, handleSelectItem]);
                ...
                return (
                    <>
                        <div className="flex-1 pb-28">
                            <Header onMenuClick={toggleMenu} onCartClick={() => setCurrentView('cart')} cartItemCount={cartItemCount} establishmentName={establishment.name} establishmentLogoUrl={establishment.logoUrl} />
                            <main>
